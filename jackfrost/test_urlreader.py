...
def test_target_filename_root_file_without_extension():
    ...
    assert (
        reader.get_target_filename(url="/testjson/", response=resp)
        == "testjson/index.js"
    )
...
def test_target_filename_without_extension_other_type():
    ...
    assert reader.get_target_filename(url="/a/b/c/", response=resp) == "a/b/c/index.js"
...
def test_target_filename_with_extension_already():
    ...
    assert (
        reader.get_target_filename(url="/a/b/c/test.js", response=resp)
        == "a/b/c/test.js"
    )
...
def test_target_filename_xmltype():
    reader = URLReader(urls=())
    resp = {"Content-Type": None}
    assert reader.get_target_filename(url="sitemap.xml", response=resp) == "sitemap.xml"
...
def test_build_page_includes_redirections():
    ...
    redirect_code = force_bytes(
        '<meta http-equiv="refresh" content="3; ' 'url={next}">'.format(
